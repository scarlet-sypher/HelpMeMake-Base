...
const mentorTimelineSchema = new mongoose.Schema(
  {
    mentorId: {
    ...
    events: [
      {
        ...
        relatedEntityType: {
          type: String,
          enum: ["Project", "Milestone", "Session"],
          default: null,
        },
        metadata: {
...
mentorTimelineSchema.methods.updateAndDetectChanges = async function (
  ...
  if (newCounts.projects) {
    ...
    if (newCounts.projects.total > oldCounts.projects.total) {
      events.push({
        type: "project_added",
        message: "A new student assigned to your mentorship",
        icon: "FolderPlus",
        color: "text-blue-400",
      });
      ...
      events.push({
        ...
        message: "A project was removed from your mentorship",
        icon: "FolderMinus",
        color: "text-red-400",
      });
    }
    ...
    if (newCounts.projects.completed > oldCounts.projects.completed) {
      events.push({
        ...
        message: "Congratulations! A project was completed successfully",
        icon: "CheckCircle",
    ...
    if (newCounts.projects.inProgress > oldCounts.projects.inProgress) {
      events.push({
        type: "project_status_changed",
        message: "A project has started and is now in progress",
        icon: "PlayCircle",
        color: "text-cyan-400",
      });
    }
  ...
  if (newCounts.milestones) {
    if (newCounts.milestones.total > oldCounts.milestones.total) {
      events.push({
        ...
        message: "A new milestone was added to your projects",
        icon: "TargetIcon",
      ...
      events.push({
        ...
        message: "A milestone was deleted from your projects",
        icon: "Trash2",
        color: "text-red-400",
      });
    }
    ...
    if (newCounts.milestones.completed > oldCounts.milestones.completed) {
      events.push({
        ...
        message: "Great! A milestone was completed",
        icon: "Target",
    ...
    if (
      newCounts.milestones.pendingReview > oldCounts.milestones.pendingReview
    ) {
      events.push({
        type: "milestone_status_changed",
        message: "A milestone is now pending your review",
        icon: "Clock",
  ...
  if (newCounts.sessions) {
    if (newCounts.sessions.total > oldCounts.sessions.total) {
      events.push({
        type: "session_added",
        message: "A new mentoring session was scheduled",
        icon: "CalendarPlus",
        color: "text-blue-400",
      });
      ...
      events.push({
        ...
        message: "A session was deleted from your calendar",
        icon: "CalendarX",
        color: "text-red-400",
      });
    }
    ...
    if (newCounts.sessions.completed > oldCounts.sessions.completed) {
      events.push({
        ...
        message: "Excellent! A mentoring session was completed",
        icon: "CheckCircle",
        color: "text-green-400",
      });
    }
...
module.exports = mongoose.model("MentorTimeline", mentorTimelineSchema);
