...
const getUserAchievements = async (req, res) => {
  try {
    ...
    let achievements = await Achievement.find({ userId }).sort({
      displayOrder: 1,
    });

    if (achievements.length === 0) {
      achievements = await Achievement.createDefaultAchievements(
        userId,
        userType
      );
    }
  ...
  try {
    ...
    const stats = {
      ...
      inProgress: achievements.filter(
        (a) => !a.isAchieved && a.criteria.currentProgress > 0
      ).length,
      byRarity: {
        common: achievements.filter(
          (a) => a.rarity === "common" && a.isAchieved
        ).length,
        rare: achievements.filter((a) => a.rarity === "rare" && a.isAchieved)
          .length,
        epic: achievements.filter((a) => a.rarity === "epic" && a.isAchieved)
          .length,
        legendary: achievements.filter(
          (a) => a.rarity === "legendary" && a.isAchieved
        ).length,
      },
