...
import { User, Shield, Link, Briefcase, Settings as SettingsIcon, Loader, Menu, Sparkles } from 'lucide-react';
...
const MentorSetting = () => {
  ...
  const handleProfileUpdate = async (e) => {
    ...
    try {
      const apiUrl = import.meta.env.VITE_API_URL || 'http://localhost:5000';
      const response = await axios.patch(`${apiUrl}/mentor/update-profile`, profileData, {
        withCredentials: true,
        headers: { 'Content-Type': 'application/json' }
      });
  ...
  const handleSocialLinksUpdate = async (e) => {
    ...
    try {
      const apiUrl = import.meta.env.VITE_API_URL || 'http://localhost:5000';
      const response = await axios.patch(`${apiUrl}/mentor/social-links`, socialLinksData, {
        withCredentials: true,
        headers: { 'Content-Type': 'application/json' }
      });
  ...
  const handlePasswordChange = async (e) => {
    ...
    try {
      const apiUrl = import.meta.env.VITE_API_URL || 'http://localhost:5000';
      const response = await axios.patch(`${apiUrl}/mentor/change-password`, {
  ...
  const handlePersonalUpdate = async (e) => {
    ...
    try {
      const apiUrl = import.meta.env.VITE_API_URL || 'http://localhost:5000';
      const response = await axios.patch(`${apiUrl}/mentor/update-personal`, personalData, {
        withCredentials: true,
        headers: { 'Content-Type': 'application/json' }
      });
      
      if (response.data.success) {
        showNotification('personal', 'success', 'Personal details updated successfully!');
        // Update local mentor data to reflect changes
        setMentorData(prev => ({
          ...prev,
      ...
      showNotification('personal', 'error', error.response?.data?.message || 'Failed to update personal details');
    } finally {
  ...
  const handleAvatarUpload = async () => {
    ...
    try {
      ...
      const apiUrl = import.meta.env.VITE_API_URL || 'http://localhost:5000';
      const response = await axios.patch(`${apiUrl}/mentor/upload-avatar`, formData, {
        withCredentials: true,
  ...
  const tabs = [
    { id: 'profile', label: 'Profile', icon: User, gradient: 'from-cyan-600 to-teal-700' },
    { id: 'social', label: 'Social Links', icon: Link, gradient: 'from-teal-700 to-cyan-700' },
    { id: 'security', label: 'Security', icon: Shield, gradient: 'from-slate-700 to-slate-600' },
    { id: 'personal', label: 'Professional', icon: Briefcase, gradient: 'from-cyan-700 to-indigo-700' }
  ];
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-slate-900 via-gray-900 to-slate-800 relative overflow-hidden">
      ...
      <div className="fixed inset-0 overflow-hidden pointer-events-none">
        ...
        {/* Floating particles */}
        <div className="absolute top-20 left-20 w-2 h-2 bg-cyan-400/40 rounded-full animate-bounce opacity-30" style={{animationDelay: '0s', animationDuration: '4s'}}></div>
        <div className="absolute top-40 right-32 w-1 h-1 bg-teal-400/40 rounded-full animate-bounce opacity-25" style={{animationDelay: '1s', animationDuration: '5s'}}></div>
        <div className="absolute bottom-32 left-40 w-3 h-3 bg-slate-400/40 rounded-full animate-bounce opacity-30" style={{animationDelay: '2s', animationDuration: '6s'}}></div>
        <div className="absolute bottom-20 right-20 w-1.5 h-1.5 bg-cyan-300/40 rounded-full animate-bounce opacity-35" style={{animationDelay: '0.5s', animationDuration: '4.5s'}}></div>
      </div>
            ...
            <div className="relative bg-white/10 backdrop-blur-xl rounded-3xl shadow-2xl p-6 lg:p-8 border border-white/20">
              <div className="flex flex-col sm:flex-row items-start sm:items-center space-y-4 sm:space-y-0 sm:space-x-6">
                ...
                <div className="flex-1">
                  ...
                  <p className="text-cyan-200/80 text-lg">Manage your mentor profile and preferences</p>
                </div>
              </div>
            </div>
          </div>
          ...
          <div className="relative group">
            ...
            <div className="relative bg-white/10 backdrop-blur-xl rounded-3xl shadow-2xl border border-white/20 overflow-hidden">
              ...
              <div className="p-6 lg:p-8">
                ...
                {activeTab === 'security' && (
                <SecurityTab
                  userData={mentorData} // Note: using mentorData instead of userData
