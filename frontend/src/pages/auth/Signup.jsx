...
const MultiSelectInput = ({ label, name, value, onChange, placeholder, required = false, error }) => (
  <div className="group">
    ...
    <textarea
      ...
      placeholder={placeholder}
      rows={3}
      className={`w-full px-4 py-3 rounded-lg bg-white/20 text-white placeholder-purple-300 focus:outline-none focus:ring-2 focus:ring-purple-500 focus:bg-white/30 transition-all duration-300 border border-white/10 resize-none ${error ? 'border-red-400 focus:ring-red-400' : ''}`}
    />
    <p className="text-purple-300 text-xs mt-1">Separate multiple items with commas</p>
    {error && <p className="text-red-400 text-sm mt-1">{error}</p>}
  </div>
);
