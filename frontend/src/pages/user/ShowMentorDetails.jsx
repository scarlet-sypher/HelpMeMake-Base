    ...
    const fetchMentorDetails = async () => {
      if (!mentor || !whyReason) {
        try {
          ...
          const mentorResponse = await axios.get(`${API_URL}/mentor/${mentorId}`, {
          ...
          if (!whyReason) {
            setWhyReason("AI reasoning not available. This mentor was recommended based on comprehensive analysis of skills, experience, and project compatibility.");
          }
    ...
    return (
      <div className="min-h-screen bg-gradient-to-br from-slate-900 via-blue-900 to-indigo-900 flex items-center justify-center">
        <div className="text-center text-white">
          ...
          <p className="text-gray-300 mb-6">The mentor you're looking for doesn't exist or is no longer available.</p>
          <button
          ...
          <div className="flex flex-col lg:flex-row lg:items-start lg:justify-between gap-6">
            ...
            <div className="flex flex-col sm:flex-row items-start space-y-4 sm:space-y-0 sm:space-x-6 flex-1">
              ...
              <div className="flex-1 text-center sm:text-left">
                ...
                <p className="text-gray-200 mb-4 leading-relaxed">{mentor.description}</p>
                ...
                <div className="flex flex-wrap justify-center sm:justify-start gap-4 mb-4">
                  <div className="flex items-center space-x-1 text-sm">
                    ...
                    <span className="text-gray-300">({mentor.totalReviews} reviews)</span>
                  </div>
                  <div className="flex items-center space-x-1 text-sm">
                    <Users className="text-green-400" size={16} />
                    <span className="text-white font-semibold">{mentor.totalStudents}</span>
                    <span className="text-gray-300">students</span>
                  </div>
                  <div className="flex items-center space-x-1 text-sm">
                    <CheckCircle className="text-blue-400" size={16} />
                    <span className="text-white font-semibold">{mentor.completedSessions}</span>
            ...
            <div className="flex flex-col space-y-3 lg:w-64">
              ...
              <button
                onClick={() => {/* Open contact modal or navigate to messages */}}
            ...
            <div className="bg-gradient-to-r from-purple-500/20 to-blue-500/20 backdrop-blur-sm rounded-3xl shadow-2xl p-6 border border-purple-500/30">
              <div className="flex items-center justify-between mb-6">
                <h2 className="text-xl font-bold text-white flex items-center">
                  ...
                  <Sparkles className="ml-2 text-yellow-400 animate-pulse" size={16} />
                </h2>
                {aiScore > 0 && (
              ...
              <div className="bg-white/10 rounded-2xl p-4 mb-4 border border-white/10">
                <div className="flex items-start space-x-3">
                  ...
                  <div className="flex-1">
                    ...
                    <p className="text-gray-200 text-sm leading-relaxed font-mono">{whyReason}</p>
                  </div>
                </div>
              </div>
                ...
                <div className="space-y-3">
                  ...
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                    {reasoningPoints.map((point, index) => (
                      <div key={index} className="bg-white/5 rounded-xl p-3 border border-white/10 flex items-start space-x-2">
                  ...
                  <div key={index} className="bg-white/10 rounded-2xl p-4 border border-white/20">
                    <div className="flex items-center justify-between mb-2">
                      ...
                      <span className={`px-2 py-1 rounded-lg text-xs font-medium ${
                        exp.level === 'expert' ? 'bg-purple-500/20 text-purple-300' :
                        exp.level === 'advanced' ? 'bg-blue-500/20 text-blue-300' :
                        exp.level === 'intermediate' ? 'bg-green-500/20 text-green-300' :
              ...
              <div className="space-y-4">
                <div className="bg-white/10 rounded-2xl p-4 border border-white/20">
                  <div className="flex items-center space-x-2 mb-2">
                    ...
                    <span className="text-white font-semibold">Total Experience</span>
                  </div>
                  <p className="text-2xl font-bold text-blue-400">{mentor.experience.years} years</p>
                </div>

                {mentor.experience.companies && mentor.experience.companies.length > 0 && (
                  <div className="space-y-3">
                    <h3 className="text-white font-semibold">Previous Companies</h3>
                    {mentor.experience.companies.map((company, index) => (
                      <div key={index} className="bg-white/5 rounded-xl p-3 border border-white/10">
                        <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between">
                          <div>
                            <h4 className="text-white font-medium">{company.position}</h4>
                        ...
                        {company.description && (
                          <p className="text-gray-200 text-sm mt-2">{company.description}</p>
                        )}
                      </div>
                    ))}
                  </div>
            ...
            <div className="bg-white/10 backdrop-blur-sm rounded-3xl shadow-2xl p-6 border border-white/20">
              ...
              <div className="space-y-4">
                ...
                <div className="flex items-center justify-between">
                  ...
                  <span className="text-white font-semibold">{mentor.profileCompleteness || 75}%</span>
                </div>
                
                <div className="flex items-center justify-between">
                  <span className="text-gray-300">Achievements</span>
                  <span className="text-white font-semibold">{mentor.achievements || 0}</span>
                </div>
              </div>
            </div>
              ...
              <div className="bg-white/10 backdrop-blur-sm rounded-3xl shadow-2xl p-6 border border-white/20">
                ...
                <div className="grid grid-cols-2 gap-3">
                  {mentor.badges.map((badge, index) => (
                    <div key={index} className="bg-white/10 rounded-xl p-3 text-center border border-white/20">
