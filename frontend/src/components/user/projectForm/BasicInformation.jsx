...
const BasicInformation = ({ formData, setFormData, errors, onToast }) => {
  const categories = [
    "Web Development",
    "Mobile Development",
    "AI/Machine Learning",
  ...
  const handleNameChange = (e) => {
    ...
    if (value.length >= 5 && value.length <= 10) {
      onToast?.({ message: "Good project name length", status: "success" });
    } else if (value.length > 150) {
      onToast?.({
        message: "Project name is getting too long",
        status: "info",
      });
    }
  };
  ...
  const handleCategoryChange = (e) => {
    ...
    if (value) {
      onToast?.({ message: `Category set to ${value}`, status: "success" });
    }
  };
  ...
  const handleDifficultyChange = (e) => {
    ...
    if (value) {
      onToast?.({
        message: `Difficulty level set to ${value}`,
        status: "success",
      });
    }
  };
