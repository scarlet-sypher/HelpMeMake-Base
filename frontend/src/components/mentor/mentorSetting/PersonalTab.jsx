...
import { 
  Briefcase, Plus, X, Clock, Globe, Star, Award, 
  Users, BookOpen, Languages, Target, Loader, CheckCircle
} from 'lucide-react';
...
const PersonalTab = ({
  ...
  const removeCompany = (index) => {
    const updatedCompanies = personalData.experience.companies.filter((_, i) => i !== index);
    setPersonalData({
      ...personalData,
      experience: {
        ...personalData.experience,
  ...
  const updateCompany = (index, field, value) => {
    const updatedCompanies = personalData.experience.companies.map((company, i) => 
  ...
  const removeExpertise = (index) => {
    const updatedExpertise = personalData.expertise.filter((_, i) => i !== index);
    setPersonalData({
      ...personalData,
  ...
  const addSpecialization = (specialization) => {
    if (specialization.trim() && !personalData.specializations.includes(specialization.trim())) {
      setPersonalData({
        ...personalData,
        specializations: [...personalData.specializations, specialization.trim()]
      });
    }
  ...
  const removeSpecialization = (index) => {
    const updatedSpecializations = personalData.specializations.filter((_, i) => i !== index);
    setPersonalData({
      ...personalData,
  ...
  const addLanguage = (language) => {
    if (language.trim() && !personalData.teachingPreferences.languages.includes(language.trim())) {
      setPersonalData({
        ...personalData,
        teachingPreferences: {
          ...personalData.teachingPreferences,
          languages: [...personalData.teachingPreferences.languages, language.trim()]
        }
      });
    }
  };
  ...
  const removeLanguage = (index) => {
    const updatedLanguages = personalData.teachingPreferences.languages.filter((_, i) => i !== index);
    setPersonalData({
      ...personalData,
      teachingPreferences: {
        ...personalData.teachingPreferences,
              ...
              {personalData.experience?.companies?.map((company, index) => (
                <div key={index} className="bg-white/5 border border-white/10 rounded-xl p-4 space-y-4">
                  <div className="flex justify-between items-start">
                    <h4 className="text-lg font-semibold text-cyan-200">Company {index + 1}</h4>
                    <button
                      type="button"
              ...
              {personalData.expertise?.map((skill, index) => (
                <div key={index} className="bg-white/5 border border-white/10 rounded-xl p-4">
                  <div className="flex justify-between items-start mb-4">
                    <h4 className="text-lg font-semibold text-teal-200">Skill {index + 1}</h4>
                    <button
                      type="button"
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                    <input
                      type="text"
                      value={skill.skill}
                      onChange={(e) => updateExpertise(index, 'skill', e.target.value)}
            ...
            <div className="space-y-4">
              <div className="flex flex-wrap gap-2">
                {personalData.specializations?.map((spec, index) => (
                  <div key={index} className="flex items-center space-x-2 bg-indigo-600/20 border border-indigo-400/30 rounded-xl px-3 py-2">
        ...
        <div className="relative group/teaching">
          ...
          <div className="relative bg-gradient-to-br from-white/10 to-white/5 rounded-2xl p-6 lg:p-8 border border-white/20">
            <div className="flex items-center space-x-3 mb-6">
              ...
              <h3 className="text-xl font-bold text-white">Teaching Preferences</h3>
            </div>
            ...
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
              <div>
                ...
                <input
                  ...
                  min="1"
                  max="10"
                  value={personalData.teachingPreferences?.maxStudentsPerSession || 1}
                  onChange={(e) => updateTeachingPreference('maxStudentsPerSession', parseInt(e.target.value) || 1)}
                ...
                <div className="space-y-2">
                  {sessionTypes.map(type => (
                    <label key={type} className="flex items-center space-x-3 text-white">
                      <input
                        type="checkbox"
                        checked={personalData.teachingPreferences?.sessionTypes?.includes(type) || false}
                        onChange={(e) => {
                          const currentTypes = personalData.teachingPreferences?.sessionTypes || [];
                          const updatedTypes = e.target.checked
                            ? [...currentTypes, type]
                            : currentTypes.filter(t => t !== type);
                          updateTeachingPreference('sessionTypes', updatedTypes);
                        }}
            ...
            <div className="mt-6">
              ...
              <div className="flex flex-wrap gap-2 mb-4">
                {personalData.teachingPreferences?.languages?.map((lang, index) => (
                  <div key={index} className="flex items-center space-x-2 bg-purple-600/20 border border-purple-400/30 rounded-xl px-3 py-2">
                    <span className="text-purple-200 font-medium">{lang}</span>
                    {personalData.teachingPreferences.languages.length > 1 && (
                      <button
                        type="button"
        ...
        <div className="relative group/availability">
          ...
          <div className="relative bg-gradient-to-br from-white/10 to-white/5 rounded-2xl p-6 lg:p-8 border border-white/20">
            <div className="flex items-center space-x-3 mb-6">
              ...
              <h3 className="text-xl font-bold text-white">Availability Settings</h3>
            </div>
        ...
        <div className="flex justify-end">
          ...
          >
            ...
            <span className="relative z-10">
              {loadingStates.personal ? 'Updating...' : 'Update Professional Details'}
            </span>
          </button>
        </div>
      </form>
