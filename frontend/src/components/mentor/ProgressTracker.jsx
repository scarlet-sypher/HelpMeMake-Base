...
import { UserCheck, User, Target, CheckCircle, Clock, Star, Crown, Anchor, Zap, Award } from 'lucide-react';
...
const ProgressTracker = ({ userImg }) => {
  ...
  const studentData = {
    ...
    progress: 75,
    image: userImg?.['luffy.jpg'] || 'https://via.placeholder.com/64x64?text=Student'
  };
  ...
  const SingleMilestone = ({ title, userVerified, mentorVerified, index, isLast, isPlaceholder }) => {
    ...
    const getStatusIcon = () => {
      ...
      if (isCompleted) return <CheckCircle size={16} className="text-white drop-shadow-lg" />;
            ...
            {!isPlaceholder && (
              <div className="flex items-center justify-center space-x-1">
                ...
                <div className="flex items-center space-x-0.5">
                  ...
                  <User size={6} className={`${mentorVerified ? 'text-purple-300' : 'text-gray-400'} transition-colors duration-300 hidden sm:block`} />
                </div>
              </div>
            )}
          </div>
      ...
      <div className="relative z-10">
        <div className="flex flex-col sm:flex-row sm:items-center justify-between mb-4 sm:mb-6 space-y-2 sm:space-y-0">
          ...
          <div className="flex items-center space-x-2">
            ...
            <span className="text-xs sm:text-sm text-cyan-300 font-medium">Live Updates</span>
          </div>
        </div>
        ...
        <div className="bg-gradient-to-r from-cyan-500/10 to-teal-500/10 rounded-2xl p-4 sm:p-6 border border-white/10 relative overflow-hidden">
          ...
          <div className="flex items-center justify-between mb-6">
            <div className="flex items-center space-x-4">
              ...
              <div>
                ...
                <p className="text-cyan-300 font-medium">{studentData.project}</p>
                <div className="flex items-center space-x-2 mt-1">
                  <div className="w-2 h-2 bg-emerald-400 rounded-full animate-pulse"></div>
                  <span className="text-sm text-emerald-300">Active Project</span>
                </div>
              </div>
            </div>
            <div className="text-right">
              <div className="text-3xl font-bold text-cyan-300">{studentData.progress}%</div>
            ...
            <div className="bg-white/10 backdrop-blur-sm rounded-xl p-3 sm:p-4 border border-white/20">
              <div className="flex items-center justify-between mb-2">
                ...
                <span className="text-xs sm:text-sm font-bold text-teal-300">
                  {(() => {
                    const completedMilestones = milestones.filter(m => m.userVerified && m.mentorVerified);
          ...
          <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
            <div className="text-center bg-white/10 rounded-xl p-4 border border-white/20">
              <div className="text-2xl font-bold text-emerald-400">{milestones.filter(m => m.userVerified && m.mentorVerified).length}</div>
              <div className="text-sm text-emerald-300">Completed</div>
            </div>
            <div className="text-center bg-white/10 rounded-xl p-4 border border-white/20">
              <div className="text-2xl font-bold text-yellow-400">{milestones.filter(m => (m.userVerified || m.mentorVerified) && !(m.userVerified && m.mentorVerified)).length}</div>
              <div className="text-sm text-yellow-300">In Progress</div>
            </div>
            <div className="text-center bg-white/10 rounded-xl p-4 border border-white/20">
              <div className="text-2xl font-bold text-slate-400">{milestones.filter(m => !m.userVerified && !m.mentorVerified).length}</div>
        ...
        <div className="mt-6 bg-white/10 backdrop-blur-sm rounded-xl p-4 border border-white/20">
          <div className="flex items-center justify-between mb-2">
            <span className="text-sm font-medium text-white">Mentorship Session Progress</span>
          ...
          <div className="w-full bg-white/20 rounded-full h-2 overflow-hidden">
            <div 
              ...
              style={{ 
                width: `${Math.round((milestones.filter(m => m.userVerified && m.mentorVerified).length / milestones.length) * 100)}%`
              }}
            ></div>
          </div>
          <div className="flex justify-between mt-2 text-xs text-blue-200">
